...
class Strip():
    """Wrapper of the Ambertools Cpptraj Strip module.
    ...
    def create_instructions_file(self):
        ...
        in_params = ''
        if in_parameters:
            in_params = get_in_parameters(self.in_parameters, self, 'strip')
...
def main():
    parser = argparse.ArgumentParser(description="Wrapper for the Ambertools cpptraj strip module.")
    ...
    Strip(input_top_path=args.input_top_path, input_traj_path=args.input_traj_path, output_cpptraj_path=args.output_cpptraj_path, properties=properties).launch()
