...
class Convert():
    ...
    Args:
        ...
        properties (dic):
            ...
            | - **input_instructions** (*dict*) - (defaults dict) Input options specification.
                ...
                | - **mask** (*string*) - (None) Mask definition. Values: C-alpha, Backbone, AllAtoms, Heavy Atoms, SideChain, Solute, Ions, Solvent.
    ...
    def create_instructions_file(self):
        ...
        if not format:
            #####################################################
            # posar 'netcdf' en funció global
            format = 'netcdf'
            fu.log('No format provided in configuration file, default value is: %s' % 'netcdf', out_log, self.global_log)
        # check if valid format
        if format not in self.formats:
            #####################################################
            fu.log('Format %s is not compatible, default value is: %s' % (format, 'netcdf'), out_log, self.global_log)
            # posar 'netcdf' en funció global
            format = 'netcdf'
