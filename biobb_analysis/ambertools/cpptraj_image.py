...
class CpptrajImage():
    ...
    Args:
        ...
        output_cpptraj_path (str): Path to the output processed trajectory. File type: output. `Sample file <https://github.com/bioexcel/biobb_analysis/raw/master/biobb_analysis/test/reference/ambertools/ref_cpptraj.image.netcdf>`_. Accepted formats: crd (edam:format_3878), netcdf (edam:format_3650), rst7 (edam:format_3886), ncrst (edam:format_2033), dcd (edam:format_3878), pdb (edam:format_1476), mol2 (edam:format_3816), binpos (edam:format_3885), trr (edam:format_3910), xtc (edam:format_3875), sqm (edam:format_2033).
    ...
    @launchlogger
    def launch(self) -> int:
        """Launches the execution of the CpptrajImage module.

        Examples:
            This is a use example of how to use the CpptrajImage module from Python

            >>> from biobb_analysis.ambertools.cpptraj_image import CpptrajImage
            >>> prop = { 'start': 1, 'end': -1, 'steps': 1, 'mask': 'c-alpha', 'format': 'netcdf' }
            >>> CpptrajImage(input_top_path='/path/to/myTopology.top', input_traj_path='/path/to/myTrajectory.dcd', output_cpptraj_path='/path/to/newTrajectory.netcdf', properties=prop).launch()
